...
class Perceptron:
	...
	def activateNOT(self, inputs: float) -> float:
		...
		weighted_sum = self.weights[0]*inputs
		#sum([self.weights[i] * inputs[i] for i in range(self.num_inputs)])

		return 1.0 if weighted_sum + self.bias > 0 else 0.0

	def validateNOT(self, examples: [float], labels: [float], verbose: bool = False) -> float:
		num_correct = 0.0

		for i in range(len(examples)):

			prediction = self.activateNOT(examples[i])
			if abs(prediction - labels[i]) < self.float_threshold:
				num_correct += 1.0
			elif verbose:
				# print('Perceptron failed on example' + str(examples[i]) + '\nPredicted: ' + str(prediction) + ' Correct Output: ' + str(labels[i]))
	...
	def trainNOT(self, examples: [float], labels: [float], learning_rate: float):
		"""NOTE: This function will run the each input example[i] through the activate function and compare its output to the label labels[i].
		if the output does not match, the perceptron learning rule is applied and each weight of the perceptron is modified in accordance with the rule."""

		for i in range(len(examples)):

			predicted_val = self.activateNOT(examples[i])


			self.learnNOT(0, predicted_val, labels[i], examples[i], learning_rate)